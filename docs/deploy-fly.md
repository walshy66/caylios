...
`caylios.com` registered. Optional: `fly mcp launch` to manage infra from Claude Code.
...
fly secrets set \
  ...
  CLERK_AUTHORIZED_PARTIES=https://coachcw.caylios.com \
...
```sh
fly certs add "*.caylios.com" --app sts-web
fly certs add caylios.com --app sts-web
fly certs add api.caylios.com --app sts-api
```

DNS at the registrar:

```
*.caylios.com   CNAME  sts-web.fly.dev
caylios.com     A/AAAA <fly IPs from `fly ips list --app sts-web`>
api.caylios.com CNAME  sts-api.fly.dev
```
...
`https://api.caylios.com` at deploy time).

## Smoke checks after deploy

1. `curl https://api.caylios.com/health` → `{"status":"ok"}`
2. `https://coachcw.caylios.com` loads the portal with Clerk sign-in
